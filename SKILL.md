...
```bash
# Today's new submissions for a category
curl -s "https://arxiv.org/rss/astro-ph"
curl -s "https://arxiv.org/rss/physics.ins-det"
curl -s "https://arxiv.org/rss/cond-mat.supr-con"
```

### RSS Structure

- Each `<item>` has `<title>`, `<link>`, `<description>` (abstract HTML), `<dc:subject>`, `<dc:creator>`
- `<dc:subject>` contains tags like `"new"`, `"cross"`, `"replaced"` — filter on `"new"` for fresh submissions only
...
for item in root.findall('.//item'):
    subjects = [s.text for s in item.findall('.//dc:subject')]
    if 'new' not in subjects:
